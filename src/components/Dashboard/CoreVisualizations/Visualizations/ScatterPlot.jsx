...
export default function RNGComparison({ freeRNG = [] }) {
  ...
  useEffect(() => {
    ...
    setData(buildScatterData(freeRNG));
  }, [freeRNG]);
  ...
  const min = Math.min(...freeRNG);
  const max = Math.max(...freeRNG);
    ...
    <div className="w-full max-w-[500px] aspect-square mx-auto -translate-y-5 md:-translate-y-0 -translate-x-5.5">
      <ResponsiveContainer width="100%" height="100%">
        <ScatterChart margin={chartMargin}>
          ...
          <Scatter name="Native RNG" data={data} fill="#3DA9FC" />
        </ScatterChart>
      </ResponsiveContainer>
    </div>
