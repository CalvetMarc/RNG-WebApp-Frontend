...
export default function VTCIImage({
  ...
  useEffect(() => {
    ...
    const applyCurve = (x) => {
      switch (curveMode) {
        case 'exp': return Math.pow(x, 2);
        case 'log': return Math.log1p(x * 9) / Math.log(10);
        case 'linear':
        default:    return x;
      }
    };
    ...
    const getRGB = (intensity) => {
      ...
      let colorStr;
      switch (colorMap) {
        case 'turbo':    colorStr = interpolateTurbo(intensity); break;
        case 'spectral': colorStr = interpolateSpectral(intensity); break;
        case 'rdBu':     colorStr = interpolateRdBu(intensity); break;
        case 'puOr':     colorStr = interpolatePuOr(intensity); break;
        case 'viridis':  colorStr = interpolateViridis(intensity); break;
        case 'inferno':  colorStr = interpolateInferno(intensity); break;
        case 'magma':    colorStr = interpolateMagma(intensity); break;
        case 'plasma':   colorStr = interpolatePlasma(intensity); break;
        case 'cividis':  colorStr = interpolateCividis(intensity); break;
        default:
    ...
    for (let i = 0; i < size * size; i++) {
      let intensity = i < total ? norm(values[i]) : 0;
  ...
  }, [values, colorMap, interpolationMode, curveMode, timeOffset]);
